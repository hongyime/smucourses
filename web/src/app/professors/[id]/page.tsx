  ...
  return (
    <div className="min-h-screen py-12 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-6xl relative z-10">
        ...
        <div className="glass-panel p-8 md:p-12 animate-fade-in relative overflow-hidden border-black/10 dark:border-white/10 bg-white/80 dark:bg-white/5">
          {/* Background Glow */}
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6 relative z-10">
            <div className="flex items-center gap-6">
              {/* Photo */}
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden bg-black/5 dark:bg-white/5 border-2 border-black/10 dark:border-white/10 flex items-center justify-center shadow-lg shrink-0">
                {photoUrl ? (
              ...
              <div>
                {/* Name */}
                <h1 className="text-3xl md:text-5xl font-display font-bold text-neutral-900 dark:text-white mb-2 leading-tight">
                  {professor.name}
                </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10 relative z-10">
            ...
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-neutral-900 dark:text-white">Contact & Profiles</h2>
              ...
              <div className="space-y-4">
                {email && (
                  <div className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300">
                    ...
                    <a href={`mailto:${email}`} className="hover:text-neutral-900 dark:hover:text-white transition-colors break-all font-medium">{email}</a>
                  </div>
                )}
                {phone && (
                  <div className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300">
                    <Phone size={16} className="text-neutral-500 shrink-0" />
                    <a href={`tel:${phone.replace(/\s/g, '')}`} className="hover:text-neutral-900 dark:hover:text-white transition-colors font-medium">{phone}</a>
                  </div>
                )}
              </div>
              ...
              {(cvUrl || profileUrl || scholarUrl || scopusUrl) && (
                <div className="space-y-3">
                  {cvUrl && (
                    <a href={cvUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors font-medium">
                      <FileText size={16} className="text-[var(--color-brand-primary)] shrink-0" />
                      <span className="font-medium">Curriculum Vitae (CV)</span>
                    </a>
                  )}
                  {profileUrl && (
                    <a href={profileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors font-medium">
                      <Globe size={16} className="text-[var(--color-brand-primary)] shrink-0" />
                      <span className="font-medium">SMU Profile Page</span>
                    </a>
                  )}
                  {scholarUrl && (
                    <a href={scholarUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors font-medium">
                      <BookOpen size={16} className="text-[var(--color-brand-primary)] shrink-0" />
                      <span className="font-medium">Google Scholar</span>
                    </a>
                  )}
                  {scopusUrl && (
                    <a href={scopusUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-neutral-700 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors font-medium">
            ...
            {qualifications && qualifications.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-neutral-900 dark:text-white mb-4">Qualifications</h2>
            ...
            {cleanResearchAreas && cleanResearchAreas.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-neutral-900 dark:text-white mb-4">Research Interests</h2>
                <ul className="list-disc pl-5 space-y-2 text-neutral-700 dark:text-neutral-300 text-sm">
            ...
            {coursesTaught && coursesTaught.length > 0 && (
              <div>
                <h2 className="text-xl font-bold text-neutral-900 dark:text-white mb-4">Courses Taught</h2>
                <ul className="list-disc pl-5 space-y-2 text-neutral-700 dark:text-neutral-300 text-sm">
          ...
          <div className="border-t border-black/10 dark:border-white/10 my-10 relative z-10"></div>
          ...
          {terms.length > 0 && (
            <div className="relative z-10">
              ...
              <div className="relative border-l border-black/10 dark:border-white/10 ml-3 space-y-12 pb-4">
                {terms.map((termKey) => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                          {termData.courses.map((course, idx) => (
                            <Link 
                              ...
                              className="bg-black/5 dark:bg-white/5 p-5 rounded-xl hover:bg-black/10 dark:hover:bg-white/10 transition-colors group flex flex-col h-full border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10"
                            >
                              ...
                              <h4 className="text-lg font-semibold text-neutral-800 dark:text-neutral-200 group-hover:text-neutral-900 dark:group-hover:text-white transition-colors leading-snug">
                                {course.courseName}
                              </h4>
                            </Link>
                          ))}
